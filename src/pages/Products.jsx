...
export default function Products() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        setItems(products.map(item => ({ ...item, featured: !!item.featured })))
      } catch (error) {
        console.log(error)
      }
    }
  ...
  const handleSearch = () => setSearch(searchText.trim())
            ...
            <div key={item._id} className="bg-[#FAFBFC] rounded-2xl overflow-hidden border border-[#E1E5EA] shadow-sm hover:shadow-md transition">
                  ...
                  >
                    {item.images.map((image, index) => (
                      <SwiperSlide key={`${item._id}-${index}`}>
                        <img src={image?.url || image} alt={item.name || "Product"} className="w-full h-52 sm:h-56 object-cover block" />
                      </SwiperSlide>
                    ))}
                  </Swiper>
                ) : (
                  <div className="w-full h-52 sm:h-56 flex items-center justify-center text-[#98A1B2] bg-[#EEF0F3]">
                    No Image
                  </div>
                )}
              ...
              <div className="p-5">
                ...
                <div className="flex items-center gap-3 mt-4">
                  ...
                  {item.discountPrice && (
                    <span className="text-[#B42318] line-through">${item.discountPrice}</span>
                  )}
                </div>

                <div className="grid grid-cols-3 gap-2 mt-5">
                  <button type="button" className="bg-[#F8FAFC] border border-[#D9DEE6] py-2.5 rounded-lg text-[#364152] hover:bg-[#F1F3F5] transition">
                    View
                  </button>
                  <button type="button" className="bg-[#F8FAFC] border border-[#D9DEE6] py-2.5 rounded-lg text-[#364152] hover:bg-[#F1F3F5] transition">
                    Edit
                  </button>
                  <button type="button" className="bg-[#263653] hover:bg-[#1D2A43] text-white py-2.5 rounded-lg transition">
                    Quick Edit
                  </button>
                </div>
              </div>
